...
/**
 * A builder for creating a complex shape without needing to know how to instantiate the
 * implementation classes of the composite.
 */
public interface Builder {
   ...
   * @throws IllegalStateException if this is not the first shape to be added but there is no
   *     current group
   */
  ...
  /**
   * Starts a group of several shapes to be treated as a single shape. Between now and the first
   * invocation of endGroup(), all new shapes will be added to this current group. Without starting
   * a group, only the most recently added shape is returned as the product.
   */
  void startGroup();

  /**
   * Ends a group of several shapes to be treated as a single shape.
   *
  ...
  /**
   * Starts a polygon consisting of several points as the current shape.
   *
   * @throws IllegalStateException if this is not the first shape to be added but there is no
   *     current group, or if there is already a current polygon
   */
  void startPolygon();
